    ...
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black bg-opacity-30">
      <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-[550px] p-6 max-h-[100vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(handleDataPost)}>
          <div className="mt-3">
            <label className="font-semibold" htmlFor="menu">
              Menu Type<span className="text-red-500">*</span>
            </label>
            <input
              placeholder="Menu Types"
              {...register("menu", { required: "Menu Types is required" })}
              id="menu"
              type="text"
              className="block w-full px-2 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-xl"
              defaultValue={menuUpdateModalValue?.menu}
            />
            {errors.menu && (
              <p className="text-red-600">{errors.menu?.message}</p>
            )}
          </div>
