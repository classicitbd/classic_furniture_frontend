...
const SubCategoryTable = ({refetch, isLoading, subCategoryTypes }) => {
    ...
    const [subCategoryUpdateModalValue, setSubCategoryUpdateModalValue] = useState(false);
            ...
            {subCategoryTypes?.data?.map((subCategory) => (
              <tr key={subCategory?._id}>
                ...
                <td className="whitespace-nowrap px-4 py-2 space-x-1 flex items-center justify-center gap-4">
                    <MdDeleteForever onClick={() =>handleDeleteCategory(subCategory)} className='cursor-pointer text-red-500 hover:text-red-300' size={25} />
                    <FiEdit onClick={() =>updateSubCategoryModal(subCategory)} className='cursor-pointer text-gray-500 hover:text-gray-300' size={25} />
                </td>
              </tr>
            ))}
          </tbody>
