...
const ProductView = ({ setIsViewOpen, isViewData }) => {
    ...
    function formatSizeVariation(option) {
        ...
        const discountString = option.discount_price ? `, Discount Price: ${option.discount_price}` : '';
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
                <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-[700px] p-6 max-h-[100vh] overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <h3 className="text-[26px] font-bold text-[#0A0A0A] capitalize" id="modal-title"> Product Information </h3>
                    ...
                    <div className='grid grid-cols-12 gap-0 md:gap-4 my-8'>
                        ...
                        <div className='md:col-span-6 col-span-12'>
                            ...
                            <h3 className='text-xl font-semibold text-bgray-900'>{isViewData?.title}</h3>

                            <div className='flex items-center justify-between mt-2'>
                                <h2 className='text-red-400'>Price: <strong>$ {isViewData?.price}</strong> </h2>
                                {
                                    isViewData?.discount_price &&
                                    <h2 className='text-red-400'>Discount Price: <strong>$ {isViewData?.discount_price}</strong> </h2>
                                }
                            </div>
                            ...
                            <h2 className='mt-1'>Category: {isViewData?.categoryId?.category} </h2>
                            <h2 className='mt-1'>Sub Category: {isViewData?.subCategoryId?.sub_category} </h2>
                            {
                                isViewData?.featureId?.feature &&
                            <h2 className='mt-1'>Feature: {isViewData?.featureId?.feature} </h2>
                            }
                            {
                                isViewData?.collectionId?.collection_name &&
                            <h2 className='mt-1'>Collection: {isViewData?.collectionId?.collection_name} </h2>
                            }
                            {
