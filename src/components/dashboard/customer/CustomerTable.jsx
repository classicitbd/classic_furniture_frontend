                                ...
                                <tbody className="divide-y divide-gray-200">
                                        <tr>
                                            ...
                                            <td className="whitespace-nowrap px-4 py-2 space-x-1 flex items-center justify-center gap-4">
                                                <MdDeleteForever onClick={() => handleDelete(isSearchData)} className='cursor-pointer text-red-500 hover:text-red-300' size={25} />
                                            </td>
                                        </tr>
                                </tbody>
                            </table>
...
{/* Show all user */}
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {users?.data?.map((user) => (
                                    <tr key={user?._id}>
                                        ...
                                        <td className="whitespace-nowrap px-4 py-2 space-x-1 flex items-center justify-center gap-4">
                                            <MdDeleteForever onClick={() => handleDelete(user)} className='cursor-pointer text-red-500 hover:text-red-300' size={25} />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {/* Pagination */}
            <Pagination rows={rows} page={page} setPage={setPage} setRows={setRows} totalData={users?.totalData} />

            {/* Handle open delete modal */}
            {
                isDeleteOpen &&
                <CustomerDeleteModal setIsSearchData={setIsSearchData} setIsSearchOpen={setIsSearchOpen} setIsDeleteOpen={setIsDeleteOpen} isDeleteData={isDeleteData} refetch={refetch} />
            }
