    ...
    const handleDataPost = (data) => {
        ...
        postFeatureType(sendData).then(result => {
            if (result?.data?.statusCode == 200 && result?.data?.success == true) {
                setLoading(false)
                toast.success(result?.data?.message ? result?.data?.message : "Feature Added successfully !", {
                    autoClose: 1000
                });
                reset();
                refetch();
                ...
                <div className="m-[30px]">
                    ...
                    <div>
                        ...
                        <form onSubmit={handleSubmit(handleDataPost)} className='flex items-center gap-2 md:gap-6 mt-3'>
                            ...
                            <div>
                                <input placeholder="Type Name" {...register("feature", { required: 'Type Name is required' })} id="feature" type="text" className="block w-full px-1 py-1 text-gray-700 bg-white border border-gray-200 rounded-xl" />
