...
const UpdateCollection = ({
  refetch,
  setCollectionUpdateModal,
  collectionUpdateModalValue,
}) => {
  const {
    register,
    reset,
      ...
      <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-[550px] p-6 max-h-[100vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button className="btn bg-white hover:bg-white border p-1">
            <RxCross1
              onClick={() => setCollectionUpdateModal(false)}
              size={25}
            ></RxCross1>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(handleDataPost)}>
          <div className="mt-3">
            <label className="font-semibold" htmlFor="collection_name">
              Collection Type<span className="text-red-500">*</span>
            </label>
            <input
              placeholder="Collection Types"
              {...register("collection_name", {
                required: "Collection Types is required",
              })}
              id="collection_name"
              type="text"
              className="block w-full px-2 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-xl"
              defaultValue={collectionUpdateModalValue?.collection_name}
            />
            {errors.collection_name && (
              <p className="text-red-600">{errors.collection_name?.message}</p>
            )}
          </div>
