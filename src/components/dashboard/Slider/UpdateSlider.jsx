...
const UpdateSlider = ({refetch, setSliderUpdateModal, sliderUpdateModalValue}) => {
    ...
    const handleDataPost = (data) => {
        ...
        updateSubCategory(formData).then(result => {
            if (result?.data?.statusCode == 200 && result?.data?.success == true) {
                toast.success(result?.data?.message ? result?.data?.message : "Slider update successfully !", {
                    autoClose: 1000
                });
                refetch();
                reset();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black bg-opacity-30">
            <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-[550px] p-6 max-h-[100vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between">
                    <h3 className="text-[26px] font-bold text-[#0A0A0A] capitalize" id="modal-title"> Update Slider </h3>
                    <button className='btn bg-white hover:bg-white border p-1'><RxCross1 onClick={() => setSliderUpdateModal(false)} size={25}></RxCross1></button>
                </div>
                ...
                <form onSubmit={handleSubmit(handleDataPost)}>
                    ...
                    <div className="mt-3">
                        <label className="font-semibold" htmlFor="slider">Slider Image<span className="text-red-500">*</span></label>
                        <input placeholder="Image" {...register("slider", { required: 'Image is required' })} id="slider" type="file" className="block w-full px-2 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-xl" />
                        {errors.slider && <p className='text-red-600'>{errors.slider?.message}</p>}
                    </div>

                    <div className="flex justify-end mt-6 gap-4">
                        <button onClick={() => setSliderUpdateModal(false)} className="btn px-6 py-2.5 transition-colors duration-300 transform bg-white rounded-xl border">Cancel</button>
